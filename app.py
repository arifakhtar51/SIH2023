...
#----
    ...
    with open("internal/discover/export_links.txt", "w") as f:
        # Loop until the urls_queue is empty
        while not urls_queue.empty():
            ...
            if link not in urls:  # To avoid Duplicates
                ...
                # Use ThreadPoolExecutor to parallelize URL checking
                future = executor.submit(requests.head, link, timeout=5, proxies=proxy)
                try:
                ...
                se = ["Ahmia","Deep Search","Onion Scanner","GDark","TORGLE","Kraken AI","Venusose","Dark Web Crawler","Shadow Search","Hidden Network"]
                search_engine = random.choice(se)
                urls.append({"url": link, "status": status, "search_engine": search_engine})
    ...
    return render_template('discover.html', api_key = api_key, secret_access_key = secret_access_key, urls = urls, total = total, search_engine = search_engine)
